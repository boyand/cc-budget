...
const EMPTY_STATE = {
  ...
  snapshot: null,  // pre-prompt snapshot, set by hook
  delta: null,
...
function updateFromStatusLine(input) {
  ...
  // Delta: compare current usage against the pre-prompt snapshot (set by hook)
  if (state.snapshot && state.rate_limits.five_hour && !windowReset) {
    const d5h = state.rate_limits.five_hour.pct - state.snapshot.five_hour_pct;
    const d7d = state.snapshot.seven_day_pct != null && state.rate_limits.seven_day
      ? state.rate_limits.seven_day.pct - state.snapshot.seven_day_pct
      : null;
    state.delta = {
  ...
  // Don't clear delta otherwise -- keep showing last known delta until next prompt
