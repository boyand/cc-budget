...
function updateFromStatusLine(input) {
  ...
  // Active model + per-model weekly windows (e.g. the separate Fable 5 limit).
  // model_scoped is undocumented statusline input; resets_at is ISO 8601 there,
  // unlike five_hour/seven_day which use epoch seconds — normalize to epoch.
  state.model = input.model?.id ?? null;
  state.model_scoped = Array.isArray(input.rate_limits?.model_scoped)
    ? input.rate_limits.model_scoped
        .filter(m => m && typeof m.utilization === 'number')
        .map(m => ({
          name: m.display_name || 'model',
          pct: m.utilization,
          resets_at: m.resets_at ? Math.floor(Date.parse(m.resets_at) / 1000) || null : null,
        }))
    : null;
