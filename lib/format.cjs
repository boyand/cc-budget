...
function progressBar(pct, width, pacePct) {
  ...
  let filledCount = Math.round((clamped / 100) * width);
  const pacePos = pacePct != null ? Math.round((Math.max(0, Math.min(100, pacePct)) / 100) * width) : -1;

  // When the pace marker falls inside the filled region, it eats a filled
  // position. Compensate so filled blocks visibly extend past the marker.
  if (pacePos >= 0 && pacePos < filledCount) {
    filledCount = Math.min(width, filledCount + 1);
  }
