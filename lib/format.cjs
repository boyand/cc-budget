  ...
  if (!fh) {
    if (state.session_cost_usd != null) {
      ...
      if (state.ledger) {
        ...
        const dayTotal = adj(entries.filter(e => e.day === today).reduce((s, e) => s + e.cost, 0));
        ...
        if (dayTotal > 0) sections.push(`${DIM}$${dayTotal.toFixed(2)} today${RESET}`);
...
function formatWidget(state, config, peak) {
  ...
  if (!fh) {
    ...
    if (state.ledger) {
      const today = new Date().toISOString().slice(0, 10);
      const dayTotal = adj(Object.values(state.ledger).filter(e => e.day === today).reduce((s, e) => s + e.cost, 0));
      if (dayTotal > 0) parts.push(`${DIM}$${dayTotal.toFixed(2)} today${RESET}`);
    }
    return parts.join(' ');
  }
