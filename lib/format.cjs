...
function formatStatusLine(state, config, peak) {
  ...
  if (!fh) {
    if (state.session_cost_usd != null) {
      return `${DIM}$${state.session_cost_usd.toFixed(2)}${RESET}`;
    }
...
module.exports = { formatStatusLine, formatResetTime, progressBar, paceTarget, formatPaceDelta, colorForPct, peakTimeRemaining, GREEN, YELLOW, RED, BRIGHT_RED, BOLD, DIM, RESET, WHITE_BOLD };
